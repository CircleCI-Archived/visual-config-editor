...
export type ReusableExecutor = { name: string, executor: AnyExecutor, type: string }

const transform = (values: any) => {
  console.log(values)
  const subtypes: { [type: string]: () => AnyExecutor } = {
    'docker': () => new executor.DockerExecutor(values.executor.image.image || 'cimg/base:stable', values.executor.resourceClass),
    'machine': () => new executor.MachineExecutor(values.executor.resourceClass, values.executor.image || 'cimg/base:latest'),
    'macos': () => new executor.MacOSExecutor(values.executor.xcode, values.executor.resourceClass),
    'windows': () => new executor.WindowsExecutor(values.executor.image, values.executor.resourceClass)
  }
...
const ExecutorMapping = (): ComponentMapping<ReusableExecutor, WorkflowJob> => {
  return {
    ...
    defaults: {
      name: 'New Executor',
      type: 'docker',
      executor: { // need to add support for subtype defaults
        image: {
    ...
    applyToNode: (data, nodeData) => {
      ...
      return { job: new Job(oldJob.name, transform({ ...data }).executor, oldJob.steps) }
    },
    components: {
      icon: ExecutorIcon,
