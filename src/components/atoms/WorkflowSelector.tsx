...
export const WorkflowSelector = () => {
  ...
  return (
    <DropdownContainer className="rounded-md mx-3 my-auto px-2 border border-circle-gray-300 hover:bg-circle-gray-300">
      ...
      <div className="rounded border border-circle-gray-300 p-2 z-30 bg-white flex flex-col">
        {Object.keys(workflows).map((workflow) => (
          <button
            value={workflow}
            key={workflow}
            className={`rounded flex w-full hover:bg-circle-gray-300 border-b border-circle-gray-300 p-2`}
            type="button"
            onClick={(e) =>
        ...
        <button
          className="tracking-wide hover:bg-circle-gray-300 leading-6 p-2 text-sm text-circle-blue font-medium"
          onClick={() =>
