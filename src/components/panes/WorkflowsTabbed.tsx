...
export interface WorkflowsTabbedProps {
    workflows: Array<WorkflowProps>;
}

const WorkflowsTabbed = (props: WorkflowsTabbedProps) => {
    return (<Tabs className="w-full h-full bg-white flex flex-col" >
        <TabList className="inline-flex pl-2 pt-2">
            {props.workflows.map((workflow) =>
                <Tab className="bg-circle-gray-200 w-max rounded-t-lg p-2 mr-2 cursor-pointer">
        ...
        <TabPanel className="flex-grow mr-4">
            {props.workflows.map((workflow) =>
                <WorkflowPane items={workflow.jobs} bgClassName="bg-circle-gray-800" className="ml-2 mr- border-2 border-b-0 border-circle-gray-400" />
            )}
        </TabPanel>
    </Tabs>)
}
